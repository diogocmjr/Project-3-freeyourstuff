  ...
  render() {  
    ...
    const item = filteredItems.map(item => (
      <div className="flex flex-col p-8 items-center" key={item._id}>
          ...
          {this.props.user ? <Link to={`/items/${item._id}`}><h1 className="text-lg mt-2 font-medium hover:underline">{item.title}</h1></Link> : <button onClick={() => this.props.updateMessage('Please log in to see item details')}><h1>{item.title}</h1></button>}
